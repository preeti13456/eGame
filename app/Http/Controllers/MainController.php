...
class MainController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($weigths->save()) {
            return redirect()
                ->route('home')
                ->with('success', 'Weigths updated successfully!');
        }
    }
        ...
        switch ($crud_path) {
            case 'users/crud_search':
                ...
                if ($users->count()) {
                    ...
                    \Session::put(
                        'error',
                        'No user was found with that data! Please, try again'
                    );
                    return back();
                }
                break;
            case 'articles/crud_search':
                ...
                if ($articles->count()) {
                    ...
                    \Session::put(
                        'error',
                        'No article was found with that data! Please, try again'
                    );
                    return back();
                }
                break;
            case 'orders/crud_search':
                ...
                if ($orders->count()) {
                    ...
                    \Session::put(
                        'error',
                        'No order was found with that data! Please, try again'
                    );
                    return back();
                }
                break;
            case 'ratings/crud_search':
                $ratings = Rating::where('id', 'like', '%' . $search . '%')
                ...
                if ($ratings->count()) {
                    ...
                    \Session::put(
                        'error',
                        'No rating was found with that data! Please, try again'
                    );
                    return back();
                }
                break;
            ...
            default:
                \Session::put(
                    'error',
                    'Something was wrong!! Please, contact with your admin server.'
                );
                return back();
                break;
        }

...
class ContentBasedFiltering
{
    ...
    public function __construct(array $articles)
    {
        ...
        $this->priceHighRange = max(
            array_column($articles, 'price')
        ); /*Nos quedamos con el precio del juego más caro*/
    }
    ...
    public function calculateSimilarityMatrix(): array
    {
        ...
        foreach ($this->articles as $article) {
            ...
            foreach ($this->articles as $_article) {
                ...
                $similarityScores[
                    'article_id_' . $_article['id']
                ] = $this->calculateSimilarityScore($article, $_article);
            }
    ...
    public function getArticlesSortedBySimilarity(
        int $articleId,
        array $matrix
    ): array {
        ...
        foreach ($similarities as $articleIdKey => $similarity) {
            ...
            $articles = array_filter($this->articles, function ($article) use (
                $id
            ) {
    ...
    protected function calculateSimilarityScore($articleA, $articleB)
    {
        return array_sum([
            ...
            Similarity::jaccard($articleA['platform'], $articleB['platform']) *
            $this->platformWeight,
            Similarity::jaccard($articleA['gender'], $articleB['gender']) *
            $this->genderWeight,
        ]) /
